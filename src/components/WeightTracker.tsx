...
export default function WeightTracker({ profile }: { profile: Profile }) {
  ...
  const fetchLogs = useCallback(async () => {
    try {
      ...
      // here rather than changing the API's default ordering.
      const sorted = [...(data.logs || [])].sort(
        (a: WeightLog, b: WeightLog) => new Date(a.logDate).getTime() - new Date(b.logDate).getTime()
      );
      setLogs(sorted);
    } catch (error) {
            ...
            <div className="flex items-center gap-3 flex-1">
              ...
              <div className="relative flex-1">
                <input
                  ...
                  step="0.1"
                  className="w-full pl-4 pr-12 py-4 input-modern rounded-xl text-white text-right text-2xl font-bold focus:outline-none"
                />
