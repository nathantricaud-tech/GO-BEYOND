...
export default function WeightTracker({ profile, onWeightLogged }: { profile: Profile; onWeightLogged?: () => void }) {
  const { lang } = useLang();
  ...
  const deleteEntry = async (logId: number) => {
    try {
      await fetch(`/api/weight-log?id=${logId}&profileId=${profile.id}`, { method: "DELETE" });
      await fetchLogs();
      onWeightLogged?.();
    } catch (error) {
  ...
  const startingWeight = logs.length > 0 ? logs[0].weight : profile.weight;
  const weightChange = logs.length >= 2 ? latestWeight - logs[0].weight : 0;
  const progress = profile.targetWeight && startingWeight !== profile.targetWeight
    ? ((startingWeight - latestWeight) / (startingWeight - profile.targetWeight)) * 100
    : 0;
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-teal-600/20 via-emerald-600/10 to-green-600/20 p-6 border border-white/10">
        ...
        <div className="relative">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white">{lang === "fr" ? "Suivi du Poids" : "Weight Tracker"}</h2>
              <p className="text-teal-300/80 text-sm">{lang === "fr" ? "Suis ta progression dans le temps" : "Track your progress over time"}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="glass rounded-2xl p-5 text-center card-hover">
          ...
          <p className="text-xs text-gray-500 mt-1">{lang === "fr" ? "Actuel" : "Current"} (kg)</p>
        </div>
        {profile.targetWeight && (
          <div className="glass rounded-2xl p-5 text-center card-hover">
            ...
            <p className="text-xs text-gray-500 mt-1">{lang === "fr" ? "Objectif" : "Target"} (kg)</p>
          </div>
        )}
        <div className="glass rounded-2xl p-5 text-center card-hover">
          ...
          <p className="text-xs text-gray-500 mt-1">{lang === "fr" ? "Évolution" : "Change"} (kg)</p>
        </div>
        {profile.targetWeight && (
          <div className="glass rounded-2xl p-5 text-center card-hover">
            ...
            <p className="text-xs text-gray-500 mt-1">{lang === "fr" ? "Progression" : "Progress"}</p>
          </div>
        )}
      </div>
        ...
        {showSuccess ? (
          <div className="flex items-center justify-center py-6 animate-scale-in">
            <div className="flex items-center gap-3 text-primary-400">
              ...
              <span className="font-semibold">{lang === "fr" ? "Poids enregistré avec succès !" : "Weight logged successfully!"}</span>
            </div>
          </div>
        ) : (
      ...
      <div className="glass rounded-2xl p-6">
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-gray-300 font-medium">{lang === "fr" ? "Aucune pesée pour l’instant" : "No weight entries yet"}</p>
            <p className="text-gray-500 text-sm mt-2">{lang === "fr" ? "Commence à te peser pour voir ton graphique de progression" : "Start logging to see your progress chart"}</p>
          </div>
        ) : (
          <div>
            {/* SVG Chart */}
            ...
            <div className="space-y-2 max-h-48 overflow-y-auto">
              {[...logs].reverse().slice(0, 10).map((log, i) => (
                <div
                  key={log.id}
                  className="flex items-center justify-between px-4 py-3 bg-white/5 rounded-xl hover:bg-white/10 transition-colors animate-slide-up group"
                  style={{ animationDelay: `${i * 30}ms` }}
                >
                  <span className="text-sm text-gray-400">{formatLogDate(log.logDate)}</span>
                  <div className="flex items-center gap-3">
                    <span className="text-sm font-semibold text-white">{log.weight} kg</span>
                    <button
                      onClick={() => deleteEntry(log.id)}
                      aria-label={lang === "fr" ? "Supprimer cette pesée" : "Delete this entry"}
                      className="w-7 h-7 rounded-full flex items-center justify-center text-gray-500 hover:text-red-400 hover:bg-red-500/10 transition-colors opacity-60 group-hover:opacity-100 active:scale-90"
                    >
                      ✕
                    </button>
                  </div>
