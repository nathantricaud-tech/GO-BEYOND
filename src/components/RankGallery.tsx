...
function monthLabel(monthKey: string, lang: "fr" | "en" = "fr") {
  const [y, m] = monthKey.split("-");
  const d = new Date(parseInt(y), parseInt(m) - 1, 1);
  const label = d.toLocaleDateString(lang === "fr" ? "fr-FR" : "en-US", { month: "long", year: "numeric" });
  return lang === "fr" ? label.charAt(0).toUpperCase() + label.slice(1) : label;
}
...
export function RankSection({ profileId }: { profileId: number }) {
  ...
  async function completeQuest() {
    ...
    try {
      const q = data.todaysQuest as unknown as Exercise;
      const mode = getExerciseDefaults(q).mode;
      const session = mode === "reps"
        ? estimateFromReps(q, q.defaultSets, getExerciseDefaults(q).defaultRepsPerSet)
        : estimateFromDuration(q, getExerciseDefaults(q).defaultDurationMin || 10);
      await fetch("/api/workout-log", {
        ...
        body: JSON.stringify({
          ...
          exerciseName: q.name,
          category: q.category,
          duration: Math.max(1, Math.round(session.durationMin)),
          caloriesBurned: session.caloriesBurned,
          sets: mode === "reps" ? q.defaultSets : null,
          reps: q.defaultReps,
  ...
  const shareBadge = useCallback(async () => {
    ...
    ctx.fillText((lang === "fr" ? data.rank.name : (RANK_NAME_EN[data.rank.key] || data.rank.name)).toUpperCase(), 400, 500);
    ...
    canvas.toBlob(async (blob) => {
      ...
      if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
        try {
          const shareRankName = lang === "fr" ? data.rank.name : (RANK_NAME_EN[data.rank.key] || data.rank.name);
          await navigator.share({ files: [file], title: lang === "fr" ? "Mon rang GO BEYOND" : "My GO BEYOND rank", text: lang === "fr" ? `Je viens d'atteindre le rang ${shareRankName} sur GO BEYOND !` : `I just reached rank ${shareRankName} on GO BEYOND!` });
          return;
        } catch {
    ...
    <div className="space-y-5">
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center pointer-events-none">
          <div className="relative">
            ...
            <div className="glass-strong rounded-3xl px-8 py-6 text-center animate-scale-in">
              ...
              <p className="text-lg font-bold" style={{ color: data.rank.color }}>{lang === "fr" ? "Nouveau rang" : "New rank"} : {lang === "fr" ? data.rank.name : (RANK_NAME_EN[data.rank.key] || data.rank.name)} !</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="animate-fade-in glass-strong rounded-3xl p-6 text-center relative overflow-hidden">
        ...
        <div className="relative">
          ...
          <p className="text-3xl font-extrabold tracking-wide" style={{ color: data.rank.color }}>{lang === "fr" ? data.rank.name : (RANK_NAME_EN[data.rank.key] || data.rank.name)}</p>
          ...
          {data.nextRank ? (
            <div className="mt-5">
              <div className="flex justify-between items-baseline text-xs text-gray-500 mb-1.5">
                <span>{lang === "fr" ? "Vers" : "To"} {lang === "fr" ? data.nextRank.name : (RANK_NAME_EN[data.nextRank.key] || data.nextRank.name)} {data.nextRank.emoji}</span>
      ...
      <div className="glass rounded-2xl p-4">
        ...
        <ul className="space-y-1.5 text-xs text-gray-400">
          <li>• <span className="text-white font-medium">+10 pts</span> — {lang === "fr" ? "dès qu\u2019une séance de sport est loggée ce jour-là" : "as soon as a workout is logged that day"}</li>
          <li>• <span className="text-white font-medium">+7 pts</span> — {lang === "fr" ? "dès qu\u2019un repas est loggé ce jour-là (les deux se cumulent : jusqu\u2019à 17 pts/jour)" : "as soon as a meal is logged that day (stacks: up to 17 pts/day)"}</li>
          ...
          {data.allRanks.map((r, i) => {
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className={`text-sm font-medium ${unlocked ? "text-white" : "text-gray-600"}`}>{lang === "fr" ? r.name : (RANK_NAME_EN[r.key] || r.name)}</p>
    ...
    <div className="space-y-5">
      <div className="glass-strong rounded-3xl p-6 text-center">
        ...
        <p className="text-gray-400 text-sm mb-4">
          {alreadyThisMonth
            ? (lang === "fr" ? "Tu as déjà ajouté ta photo pour ce mois-ci — reviens le mois prochain !" : "You already added your photo for this month — come back next month!")
            : (lang === "fr" ? "Débloque ta photo de progression de ce mois pour l'ajouter à ta galerie." : "Unlock this month's progress photo to add it to your gallery.")}
        </p>
        ...
        >
          {alreadyThisMonth ? (lang === "fr" ? "Remplacer la photo de ce mois" : "Replace this month's photo") : (lang === "fr" ? "Prendre / choisir une photo" : "Take / choose a photo")}
        </button>
      </div>
      ...
      {preview && (
        <div className="fixed inset-0 bg-black/85 backdrop-blur-md z-50 flex items-center justify-center p-4" onClick={() => setPreview(null)}>
          <div className="glass-strong rounded-3xl p-5 max-w-sm w-full" onClick={(e) => e.stopPropagation()}>
            <img src={preview} alt={lang === "fr" ? "Aperçu" : "Preview"} className="w-full rounded-2xl mb-4 max-h-80 object-cover" />
            <div className="flex gap-3">
              ...
              >
                {uploading ? "..." : (lang === "fr" ? "Confirmer" : "Confirm")}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-50 flex items-center justify-center p-4" onClick={() => setViewerPhoto(null)}>
          <div className="max-w-lg w-full" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-3">
              <p className="text-white font-medium">{monthLabel(viewerPhoto.monthKey, lang)}</p>
            ...
            <img src={viewerPhoto.photoData} alt={monthLabel(viewerPhoto.monthKey, lang)} className="w-full rounded-2xl" />
          </div>
        </div>
      )}

      <div>
        <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-3">{lang === "fr" ? "Historique" : "History"} ({photos.length})</h3>
        {photos.length === 0 ? (
          <div className="text-center py-10 glass rounded-2xl">
            <span className="text-3xl block mb-2">🗂️</span>
            <p className="text-gray-500 text-sm">{lang === "fr" ? "Aucune photo pour l'instant" : "No photos yet"}</p>
          </div>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {photos.map((p) => (
              ...
              >
                <img src={p.photoData} alt={monthLabel(p.monthKey, lang)} className="w-full h-full object-cover" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent" />
                <span className="absolute bottom-2 left-2 text-[11px] text-white font-medium">{monthLabel(p.monthKey, lang)}</span>
              </button>
            ))}
          </div>
        )}
