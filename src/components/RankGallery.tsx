...
export function RankSection({ profileId }: { profileId: number }) {
  ...
  useEffect(() => {
    fetchRank().catch(() => {}).finally(() => setLoading(false));
  }, [fetchRank]);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="glass rounded-2xl p-4">
        ...
        <p className="text-[11px] text-gray-500 mt-2">Les points sont cumulés à vie, rien n&apos;est jamais perdu. Monter de rang devient progressivement plus long : quelques jours pour les premiers rangs, plusieurs semaines d&apos;assiduité pour atteindre HERO.</p>
      </div>
      ...
      <div className="glass-strong rounded-3xl p-5 relative overflow-hidden">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-white font-semibold">{data.todaysQuest.name}</p>
            <p className="text-xs text-gray-500 mt-0.5">
              {data.todaysQuest.defaultSets}×{data.todaysQuest.defaultReps} · {data.todaysQuest.difficulty}
            </p>
          </div>
        </div>
        <button
