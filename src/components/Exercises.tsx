...
export default function Exercises({ userGoal, profileId }: Props) {
  const { lang } = useLang();
  const exercises = allExercises.map((e) => translateExercise(e, lang));
  const workoutPrograms = allWorkoutPrograms.map((p) => translateProgram(p, lang));
    ...
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-purple-600/20 via-pink-600/10 to-rose-600/20 p-6 border border-white/10">
        ...
        <div className="relative flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-purple-300/80 text-sm">{exercises.length} exercices · {workoutPrograms.length} programmes</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex p-1 bg-white/5 rounded-2xl">
        {(["programs", "exercises"] as const).map(t => (
          <button key={t} onClick={() => setTab(t)} className={`flex-1 py-2.5 rounded-xl text-sm font-medium transition-all capitalize ${tab === t ? "bg-gradient-to-r from-primary-500 to-primary-600 text-white shadow-lg shadow-primary-500/25" : "text-gray-400"}`}>{t === "programs" ? "📋 Programmes" : "💪 Exercices"}</button>
        ))}
      </div>
        ...
        <div className="space-y-4">
          ...
          {!selectedProg && recPrograms.length > 0 && (
            <div className="glass rounded-2xl p-4 border border-primary-500/20">
              <p className="text-sm font-semibold text-primary-400 mb-3">✨ Recommandé pour toi</p>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex gap-3 mt-1 text-xs text-gray-400">
                      <span>{selectedProg.daysPerWeek}x/semaine</span>
            ...
            <div className="space-y-3">
              <div className="flex flex-col gap-2">
                <div className="flex items-center justify-between">
                  <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider">Tous les programmes</h3>
      ...
      {tab === "exercises" && (
        <div className="space-y-4">
          ...
          {/* Filters */}
          <div className="flex gap-2 overflow-x-auto pb-1">
            {[{ v: "all", l: "Tout" }, { v: "strength", l: "💪 Muscu" }, { v: "calisthenics", l: "🤸 Poids du corps" }, { v: "cardio", l: "❤️ Cardio" }, { v: "hiit", l: "🔥 HIIT" }, { v: "flexibility", l: "🧘 Souplesse" }].map(f => (
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            {[{ v: "all", l: "Tous les muscles" }, ...muscleGroups.map(m => ({ v: m, l: m }))].map(f => (
              ...
              <div className="glass-strong rounded-3xl w-full max-w-lg max-h-[90vh] overflow-y-auto animate-scale-in relative" onClick={e => e.stopPropagation()}>
                ...
                <div className="p-6">
                  ...
                  <div className="grid grid-cols-3 gap-3 mb-5">
                    ...
                    <div className="bg-white/5 rounded-xl p-3 text-center"><span className="text-lg font-bold text-blue-400">{selectedEx.defaultSets}×{selectedEx.defaultReps}</span><p className="text-xs text-gray-500">Séries × Reps</p></div>
                  ...
                  <div className="mb-5"><p className="text-xs font-semibold text-gray-400 uppercase mb-2">Muscles sollicités</p><div className="flex flex-wrap gap-1.5">{selectedEx.muscles.map(m => <span key={m} className="text-xs px-2 py-1 bg-white/5 rounded-lg text-gray-300">{m}</span>)}</div></div>
                  <div className="mb-5">
                    <p className="text-xs font-semibold text-gray-400 uppercase mb-3">Comment l’exécuter</p>
                  ...
                  <div className="mb-5"><p className="text-xs font-semibold text-gray-400 uppercase mb-2">💡 Astuces</p>{selectedEx.tips.map((t, i) => <p key={i} className="text-sm text-gray-400 mb-1.5">• {t}</p>)}</div>
                      ...
                      <div className="mb-5 bg-white/5 rounded-2xl p-4">
                        ...
                        {mode === "reps" ? (
                          <div className="grid grid-cols-2 gap-3">
                            <div>
                              <p className="text-[11px] text-gray-500 mb-1.5">Séries</p>
                            ...
                            <div>
                              <p className="text-[11px] text-gray-500 mb-1.5">Reps / série</p>
                              <div className="flex items-center gap-2">
                          ...
                          <div>
                            <p className="text-[11px] text-gray-500 mb-1.5">Durée (minutes)</p>
                            <div className="flex items-center gap-2">
          ...
          {filtered.length === 0 && <div className="text-center py-12 glass rounded-2xl"><span className="text-4xl block mb-3">🔍</span><p className="text-gray-400">Aucun exercice ne correspond aux filtres</p></div>}
        </div>
      )}
    </div>
  );
