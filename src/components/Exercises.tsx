                ...
                <div key={di} className="mb-4">
                  ...
                  <div className="ml-10 space-y-1.5">
                    {day.exercises.map((we, ei) => {
                      ...
                      return (
                        <div key={ei} className={`flex items-center justify-between px-3 py-2.5 rounded-xl transition-all ${done ? "bg-primary-500/10 border border-primary-500/20" : "bg-white/5 hover:bg-white/10"}`}>
                          <div className="flex items-center gap-3">
                          ...
                          <button onClick={() => logExercise(ex)} disabled={done} className={`px-3 py-1 rounded-lg text-xs font-medium transition-all ${done ? "bg-primary-500/20 text-primary-400" : "bg-white/10 text-gray-400 hover:bg-primary-500/20 hover:text-primary-400"}`}>
                            {done ? "✓ Done" : "Log"}
                          </button>
                        </div>
                      );
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex gap-3 mt-2 text-xs">
                        <span className="text-gray-400">{p.daysPerWeek}x/week</span>
        ...
        <div className="space-y-4">
          ...
          {selectedEx && (
            <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-end sm:items-center justify-center p-4" onClick={() => setSelectedEx(null)}>
              <div className="glass-strong rounded-3xl w-full max-w-lg max-h-[90vh] overflow-y-auto animate-scale-in" onClick={e => e.stopPropagation()}>
                <div className="p-6 pb-0 flex flex-col items-center">
                  <button onClick={() => setSelectedEx(null)} className="absolute top-4 right-4 w-10 h-10 rounded-full bg-white/10 flex items-center justify-center text-gray-400 z-10">✕</button>
                  <div className="relative mb-4">
                    <ExerciseAnimation emoji={selectedEx.emoji} category={selectedEx.category} size={100} />
                  </div>
