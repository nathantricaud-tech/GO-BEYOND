...
export default function MealPlanner({ profile }: { profile: Profile }) {
  ...
  const recipesByName = useMemo(() => {
    ...
    for (const r of allRecipes) map.set(r.name, r);
    return map;
  }, []);
  ...
  const PLAN_NAME_EN: Record<string, string> = {
    "Plan Prise de Muscle": "Muscle Building Plan",
    "Plan Perte de Poids": "Weight Loss Plan",
    "Plan Équilibré": "Balanced Plan",
  };
  const displayPlanName = (name: string) => (lang === "fr" ? name : PLAN_NAME_EN[name] || name);

  const DESC_RE = /Plan sur mesure ~(\d+) kcal\/jour \(cible (\d+) kcal\)\. Objectif macros ?: (\d+)g protéines, (\d+)g glucides, (\d+)g lipides\./;
  const displayPlanDescription = (description: string | null) => {
    if (!description) return "";
    if (lang === "fr") return description;
    const m = description.match(DESC_RE);
    if (!m) return description;
    const [, total, target, protein, carbs, fat] = m;
    return `Custom plan ~${total} kcal/day (target ${target} kcal). Macro goals: ${protein}g protein, ${carbs}g carbs, ${fat}g fat.`;
  };
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className={`relative overflow-hidden rounded-3xl bg-gradient-to-br ${goalInfo.gradient}/20 p-6 border border-white/10`}>
        ...
        <div className="relative flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white">{lang === "fr" ? "Plan Alimentaire" : "Meal Planner"}</h2>
              <p className="text-gray-400 text-sm">
                {profile.dailyCalorieTarget || 2000} kcal/{lang === "fr" ? "jour" : "day"} • {goalInfo.label}
              </p>
            </div>
          </div>
          <button
          ...
          >
            {generating ? (
              ...
              <>
                <span>✨</span>
                {lang === "fr" ? "Générer un plan" : "Generate Plan"}
              </>
            )}
          </button>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 glass rounded-3xl">
          ...
          <h3 className="text-xl font-bold text-white mb-2">{lang === "fr" ? "Aucun Plan Pour l’instant" : "No Meal Plans Yet"}</h3>
          <p className="text-gray-400 max-w-sm mx-auto">
            {lang === "fr" ? "Génère ton premier plan alimentaire personnalisé selon tes objectifs et tes besoins caloriques." : "Generate your first personalized meal plan based on your goals and daily calorie needs."}
          </p>
        </div>
      ) : (
            ...
            >
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-white">{displayPlanName(plan.name)}</h3>
                    <p className="text-sm text-gray-500 mt-0.5 max-w-md truncate">{displayPlanDescription(plan.description)}</p>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                ...
                <div className="px-5 pb-5 border-t border-white/5 pt-4 animate-fade-in">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
                    {[
                      ...
                      { label: lang === "fr" ? "Protéines" : "Protein", value: Math.round(plan.totalProtein || 0), unit: "g", gradient: "from-blue-500 to-cyan-500" },
                      { label: lang === "fr" ? "Glucides" : "Carbs", value: Math.round(plan.totalCarbs || 0), unit: "g", gradient: "from-amber-500 to-orange-500" },
                      { label: lang === "fr" ? "Lipides" : "Fat", value: Math.round(plan.totalFat || 0), unit: "g", gradient: "from-pink-500 to-rose-500" },
                    ].map((stat) => (
                    ...
                    return (
                      <div key={type} className="mb-4 last:mb-0">
                        ...
                        <div className="space-y-1.5 ml-10">
                          {items.map((item) => {
                            const matchedRaw = recipesByName.get(item.foodName);
                            ...
                            return (
                              ...
                              >
                                <span className="text-sm text-gray-300 flex items-center gap-2">
                                  ...
                                  {matchedRecipe ? matchedRecipe.name : item.foodName}
                                </span>
            ...
            <div className="p-6">
              ...
              <div className="grid grid-cols-3 gap-3 mb-6">
                <div className="bg-white/5 rounded-xl p-3 text-center">
                  ...
                  <p className="text-[10px] text-gray-500">{lang === "fr" ? "Protéines" : "Protein"}</p>
                </div>
                <div className="bg-white/5 rounded-xl p-3 text-center">
                  <p className="text-lg font-bold text-white">{selectedRecipe.carbs}g</p>
                  <p className="text-[10px] text-gray-500">{lang === "fr" ? "Glucides" : "Carbs"}</p>
                </div>
                <div className="bg-white/5 rounded-xl p-3 text-center">
                  <p className="text-lg font-bold text-white">{selectedRecipe.fat}g</p>
                  <p className="text-[10px] text-gray-500">{lang === "fr" ? "Lipides" : "Fat"}</p>
                </div>
              </div>
              <h4 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-2">{lang === "fr" ? "Ingrédients" : "Ingredients"}</h4>
              ...
              <h4 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-2">{lang === "fr" ? "Préparation" : "Instructions"}</h4>
