        ...
        <div className="glass rounded-2xl p-5 border border-primary-500/20">
          ...
          <div className="flex gap-4 overflow-x-auto pb-2 -mx-1 px-1 snap-x">
            {recommendedRecipes.map((recipe, i) => (
              ...
              >
                <div
                  className={`h-32 bg-gradient-to-br ${getRecipeVisual(recipe).gradient} flex items-center justify-center group-hover:scale-110 transition-transform duration-500`}
                >
                  <span className="text-4xl">{getRecipeVisual(recipe).emoji}</span>
                </div>
      ...
      {selectedRecipe && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-end sm:items-center justify-center p-4" onClick={() => setSelectedRecipe(null)}>
          <div className="glass-strong rounded-3xl w-full max-w-lg max-h-[90vh] overflow-y-auto animate-scale-in relative" onClick={e => e.stopPropagation()}>
            ...
            {/* Visual */}
            <div className={`relative h-44 -mt-[52px] bg-gradient-to-br ${getRecipeVisual(selectedRecipe).gradient} flex items-center justify-center`}>
              <span className="text-7xl drop-shadow-lg">{getRecipeVisual(selectedRecipe).emoji}</span>
        ...
        {filteredRecipes.map((recipe, i) => (
          ...
          >
            <div className="relative h-40 overflow-hidden">
              <div
                className={`absolute inset-0 bg-gradient-to-br ${getRecipeVisual(recipe).gradient} flex items-center justify-center group-hover:scale-110 transition-transform duration-500`}
              >
                <span className="text-5xl">{getRecipeVisual(recipe).emoji}</span>
              </div>
