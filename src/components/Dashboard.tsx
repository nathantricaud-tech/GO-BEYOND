...
export default function Dashboard({ profile }: { profile: Profile }) {
  ...
  const deleteWorkout = async (id: number) => {
    try {
      await fetch(`/api/workout-log?id=${id}`, { method: "DELETE" });
      await fetchAll();
    } catch (error) {
  ...
  const getBMIInfo = (b: number) => {
    if (b < 18.5) return { label: lang === "fr" ? "Maigreur" : "Underweight", color: "text-blue-400", bg: "bg-blue-500/20" };
    if (b < 25) return { label: lang === "fr" ? "Normal" : "Normal", color: "text-green-400", bg: "bg-green-500/20" };
    if (b < 30) return { label: lang === "fr" ? "Surpoids" : "Overweight", color: "text-yellow-400", bg: "bg-yellow-500/20" };
    return { label: lang === "fr" ? "Obésité" : "Obese", color: "text-red-400", bg: "bg-red-500/20" };
  };
  const bmiInfo = getBMIInfo(bmi);

  const goalEmoji = profile.goal === "lose_weight" ? "🔥" : profile.goal === "gain_muscle" ? "💪" : "⚖️";
  const goalLabel = lang === "fr"
    ? (profile.goal === "lose_weight" ? "Perte de poids" : profile.goal === "gain_muscle" ? "Prise de muscle" : "Maintien")
    : (profile.goal === "lose_weight" ? "Lose Weight" : profile.goal === "gain_muscle" ? "Build Muscle" : "Maintain");
    ...
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-primary-600/30 via-primary-500/20 to-teal-600/10 p-6 border border-primary-500/20">
        ...
        <div className="relative">
          <div className="min-w-0">
            ...
            <h2 className="text-2xl sm:text-3xl font-bold text-white mt-1 truncate">{profile.name} <span className="inline-block animate-bounce-subtle">👋</span></h2>
            <p className="text-gray-300 mt-2 flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-primary-400 animate-pulse" />
              <span className="text-gradient font-semibold">{remaining > 0 ? remaining : 0}</span> kcal {lang === "fr" ? "restantes" : "remaining"}
            </p>
            <div className="glass px-4 py-2.5 rounded-2xl flex items-center gap-2 w-fit mt-3">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="glass rounded-2xl p-4 text-center card-hover">
          ...
          <p className="text-xs text-gray-500 mt-0.5">{lang === "fr" ? "Exercices" : "Exercises"}</p>
        </div>
        {/* Calories Burned */}
        <div className="glass rounded-2xl p-4 text-center card-hover">
          ...
          <p className="text-xs text-gray-500 mt-0.5">kcal {lang === "fr" ? "brûlées" : "burned"}</p>
        </div>
        {/* Workout Minutes */}
        <div className="glass rounded-2xl p-4 text-center card-hover">
          ...
          <p className="text-xs text-gray-500 mt-0.5">min {lang === "fr" ? "d’entraînement" : "trained"}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="glass rounded-3xl p-6 flex flex-col items-center justify-center card-hover">
          ...
          <p className="text-sm text-gray-400 mt-3 font-medium">{lang === "fr" ? "Calories Consommées" : "Calories Intake"}</p>
        </div>
        ...
        <div className="glass rounded-3xl p-6 card-hover">
          <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest flex items-center gap-2 mb-4">
            ...
            {lang === "fr" ? "Suivi de l’Eau" : "Water Tracker"}
          </h3>
          <div className="flex items-center justify-center gap-4 mb-4">
            ...
            <div className="text-center">
              ...
              <span className="text-gray-500 text-sm block">/ {waterTarget} {lang === "fr" ? "verres" : "glasses"}</span>
            </div>
        ...
        <div className="glass rounded-3xl p-6 card-hover">
          ...
          <div className="space-y-2.5">
            ...
            {[
              { label: lang === "fr" ? "Poids" : "Weight", value: `${profile.weight} kg`, icon: "⚖️" },
              { label: lang === "fr" ? "Taille" : "Height", value: `${profile.height} cm`, icon: "📏" },
              ...(profile.targetWeight ? [{ label: lang === "fr" ? "Objectif" : "Target", value: `${profile.targetWeight} kg`, icon: "🎯" }] : []),
            ].map(s => (
      ...
      <div className="glass rounded-3xl p-6 card-hover">
        <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest flex items-center gap-2 mb-5">
          ...
          {lang === "fr" ? "Macros du Jour" : "Today’s Macros"}
        </h3>
        <div className="grid grid-cols-3 gap-4">
          {[
            { label: lang === "fr" ? "Protéines" : "Protein", value: totalProtein, target: macros.protein, color: "from-blue-500 to-cyan-500", emoji: "🥩" },
            { label: lang === "fr" ? "Glucides" : "Carbs", value: totalCarbs, target: macros.carbs, color: "from-amber-500 to-orange-500", emoji: "🍞" },
            { label: lang === "fr" ? "Lipides" : "Fat", value: totalFat, target: macros.fat, color: "from-pink-500 to-rose-500", emoji: "🥑" },
          ].map(m => (
            <div key={m.label}>
      ...
      {todayWorkouts.length > 0 && (
        <div className="glass rounded-3xl p-6">
          <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest flex items-center gap-2 mb-4">
            ...
            {lang === "fr" ? "Entraînement du Jour" : "Today’s Training"}
          </h3>
          <div className="space-y-2">
            {todayWorkouts.map(w => (
              <div key={w.id} className="flex items-center justify-between px-4 py-3 bg-white/5 rounded-xl group">
                <div className="flex items-center gap-3 min-w-0">
                  <span className="text-lg">🏋️</span>
                  <span className="text-sm text-gray-300 truncate">{w.exerciseName}</span>
                </div>
                <div className="flex items-center gap-3 text-xs text-gray-500 flex-shrink-0">
                  ...
                  <button
                    ...
                    aria-label={lang === "fr" ? "Supprimer cet exercice" : "Delete this exercise"}
                    className="w-6 h-6 rounded-full flex items-center justify-center text-gray-500 hover:text-red-400 hover:bg-red-500/10 transition-colors opacity-60 group-hover:opacity-100 active:scale-90"
                  >
                    ✕
                  </button>
                </div>
