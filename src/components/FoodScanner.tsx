    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {showCamera && (
        <div className="fixed inset-0 z-50 bg-black flex flex-col">
          ...
          <div className="absolute top-0 left-0 right-0 z-20 p-4 bg-gradient-to-b from-black/80 to-transparent flex items-center justify-between">
            ...
            <span className="text-white font-medium">{lang === "fr" ? "Scanne ton Repas" : "Scan Your Food"}</span>
            <div className="w-10" />
          </div>
          ...
          {analyzing && (
            <div className="flex-1 flex flex-col items-center justify-center relative">
              ...
              <div className="relative z-10 flex flex-col items-center">
                ...
                <p className="text-white text-lg font-medium mt-6">{lang === "fr" ? "Analyse des couleurs & formes…" : "Analyzing colors & shapes…"}</p>
                <p className="text-gray-400 text-sm mt-2">{lang === "fr" ? "Détection du type d’aliment" : "Detecting food type"}</p>
              </div>
            </div>
          )}
          ...
          {showGuesses && (
            <div className="flex-1 flex flex-col relative">
              ...
              <div className="relative z-10 p-5 bg-gradient-to-t from-black via-black/95 to-black/60 rounded-t-3xl">
                <h3 className="text-white font-bold text-lg mb-1">{lang === "fr" ? "Qu’avons-nous détecté ?" : "What did we detect?"}</h3>
                <p className="text-gray-400 text-sm mb-4">{lang === "fr" ? "Choisis l’aliment qui correspond à ta photo" : "Pick the food that matches your photo"}</p>
          ...
          {cameraError && !analyzing && !showGuesses && (
            <div className="flex-1 flex flex-col items-center justify-center px-6">
              ...
              <h3 className="text-xl font-bold text-white text-center">{lang === "fr" ? "Caméra indisponible" : "Camera not available"}</h3>
              <p className="text-gray-400 text-center mt-2 max-w-xs">{lang === "fr" ? "Ton navigateur a bloqué l’accès à la caméra. Importe une photo à la place." : "Your browser blocked camera access. Upload a photo instead."}</p>
              ...
              <button onClick={stopCamera} className="mt-3 text-gray-500 hover:text-white text-sm transition-colors">{lang === "fr" ? "Annuler" : "Cancel"}</button>
            </div>
          )}
            ...
            <>
              ...
              {cameraReady && (
                <div className="absolute bottom-0 left-0 right-0 p-8 bg-gradient-to-t from-black/80 to-transparent">
                  ...
                  <p className="text-center text-white/60 text-sm mt-4">{lang === "fr" ? "Vise ton plat et appuie pour capturer" : "Point at your food and tap to capture"}</p>
                </div>
              )}
              {!cameraReady && (
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-blue-600/20 via-cyan-600/10 to-teal-600/20 p-6 border border-white/10">
        ...
        <div className="relative flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-bold text-white">{lang === "fr" ? "Scanner Alimentaire" : "Food Scanner"}</h2>
            <p className="text-blue-300/80 text-sm">{lang === "fr" ? "Scanne ou recherche pour suivre ta nutrition" : "Scan or search to track nutrition"}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <button onClick={startCamera} className="relative overflow-hidden group rounded-2xl p-5 bg-gradient-to-r from-primary-600 to-primary-500 shadow-xl shadow-primary-500/25 transition-all hover:shadow-primary-500/40 hover:scale-[1.02] active:scale-[0.98]">
          ...
          <div className="relative flex flex-col items-center gap-2 text-center">
            ...
            <span className="text-white font-semibold">{lang === "fr" ? "Ouvrir la Caméra" : "Open Camera"}</span>
            <span className="text-primary-100/80 text-xs">{lang === "fr" ? "Prendre une photo" : "Take a photo"}</span>
          </div>
        </button>
        <button onClick={() => fileInputRef.current?.click()} className="relative overflow-hidden group rounded-2xl p-5 glass hover:bg-white/10 transition-all hover:scale-[1.02] active:scale-[0.98]">
          <div className="relative flex flex-col items-center gap-2 text-center">
            <span className="text-3xl">📁</span>
            <span className="text-white font-semibold">{lang === "fr" ? "Importer une Photo" : "Upload Photo"}</span>
            <span className="text-gray-400 text-xs">{lang === "fr" ? "Depuis la galerie" : "From gallery"}</span>
          </div>
        </button>
      </div>
      ...
      {results.length > 0 && !selectedFood && (
        <div className="space-y-3 animate-slide-up">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setResults([])} className="text-xs text-gray-500 hover:text-white transition-colors">{lang === "fr" ? "Effacer" : "Clear"}</button>
          </div>
          {results.map((food, i) => (
      ...
      {results.length === 0 && !loading && query && !showCamera && (
        <div className="text-center py-12 glass rounded-2xl animate-fade-in">
          ...
          <p className="text-gray-500 text-sm mt-2">{lang === "fr" ? "Essaie : pomme, poulet, riz, saumon, pâtes, pizza…" : "Try: apple, chicken, rice, salmon, pasta, pizza…"}</p>
        </div>
      )}
        ...
        <div className="glass rounded-3xl p-6 animate-scale-in">
          {addSuccess ? (
            <div className="text-center py-8">
              ...
              <h3 className="text-2xl font-bold text-white">{lang === "fr" ? "Ajouté !" : "Added!"}</h3>
            ...
            <>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
                {[
                  ...
                  { label: lang === "fr" ? "Protéines" : "Protein", value: Math.round(selectedFood.protein * servings), unit: "g", color: "from-blue-500 to-cyan-500" },
                  { label: lang === "fr" ? "Glucides" : "Carbs", value: Math.round(selectedFood.carbs * servings), unit: "g", color: "from-amber-500 to-orange-500" },
                  { label: lang === "fr" ? "Lipides" : "Fat", value: Math.round(selectedFood.fat * servings), unit: "g", color: "from-pink-500 to-rose-500" },
                ].map((s) => (
              ...
              <div className="flex items-center justify-between mb-6 p-4 bg-white/5 rounded-2xl">
                <span className="text-gray-400">{lang === "fr" ? "Portions" : "Servings"}</span>
                <div className="flex items-center gap-3">
              ...
              <div className="mb-6">
                <p className="text-sm text-gray-400 mb-3">{lang === "fr" ? "Ajouter au repas" : "Add to meal"}</p>
              ...
              <button onClick={handleAddToLog} className="w-full py-4 btn-primary rounded-2xl text-white font-semibold text-lg">{lang === "fr" ? "Ajouter au Journal" : "Add to Food Log"}</button>
            </>
          )}
        </div>
      )}
      ...
      {!selectedFood && results.length === 0 && !showCamera && (
        <div className="glass rounded-2xl p-5">
          <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-4">{lang === "fr" ? "Aliments Populaires" : "Popular Foods"}</h3>
