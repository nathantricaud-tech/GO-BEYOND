...
export default function Home() {
  ...
  const bgTintByTab: Record<TabType, string> = {
    dashboard: "bg-mesh-dashboard",
    training: "bg-mesh-training",
    nutrition: "bg-mesh-nutrition",
    recipes: "bg-mesh-recipes",
    progress: "bg-mesh-progress",
  };

  return (
    <div className={`min-h-screen bg-mesh ${bgTintByTab[activeTab]} pb-24 sm:pb-6`}>
      <AvatarTheme avatarId={profile.avatar} />
      ...
      <main key={activeTab} className="max-w-4xl mx-auto px-4 py-6 tab-transition">
        ...
        {activeTab === "progress" && <ProgressHub profile={profile} onWeightLogged={checkSession} />}
      </main>
      ...
      <ReflexGame profileId={profile.id} />
    </div>
  );
}
