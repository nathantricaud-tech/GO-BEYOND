...
const WORKOUT_DAY_POINTS = 10;
const NUTRITION_DAY_POINTS = 7;
const FULL_DAY_POINTS = WORKOUT_DAY_POINTS + NUTRITION_DAY_POINTS; // both logged same day
export const QUEST_BONUS_POINTS = 3;
...
export function computePointsFromDates(foodDates: Set<string>, workoutDates: Set<string>): number {
  ...
  for (const date of allDates) {
    if (workoutDates.has(date)) points += WORKOUT_DAY_POINTS;
    if (foodDates.has(date)) points += NUTRITION_DAY_POINTS;
  }
  return points;
}
...
export const RANK_NAME_EN: Record<string, string> = {
  novice: "Novice",
  fer: "Iron",
  bronze: "Bronze",
  argent: "Silver",
...
export function translateRankName(rank: RankDef, lang: "fr" | "en"): string {
  return lang === "fr" ? rank.name : RANK_NAME_EN[rank.key] || rank.name;
}
